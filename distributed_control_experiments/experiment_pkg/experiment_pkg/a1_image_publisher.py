...
#    )
    ...
    if len(sys.argv)>1:
        ...
        if len(sys.argv) > 2:
            ...
            if len(sys.argv) > 3:
                fps = int(sys.argv[3])
                if len(sys.argv) == 5:
                    res_id = int(sys.argv[4])
                    image_width = image_widths[res_id]
                    image_height = image_heights[res_id]
                elif len(sys.argv) > 5:
    ...
    self.publisher_ = self.create_publisher(ImageStampId, 'camera/image_raw', qos_profiles_dict[self.qos_profile])#,event_callbacks=self.publisher_callbacks
