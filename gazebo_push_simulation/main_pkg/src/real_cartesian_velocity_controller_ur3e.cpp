...
class CartesianVelocityPublisher : public rclcpp::Node
{
  public:
    CartesianVelocityPublisher() : Node("real_cartesian_velocity_publisher")
    {
      ...
      service_ = this->create_service<custom_interfaces::srv::RobPose>("/real_ur3e_velocity_controller/set_desired_rob_pose", std::bind(&CartesianVelocityPublisher::set_desired_rob_pose, this, _1, _2),rmw_qos_profile_services_default,service_cb_group_);
  ...
  private:
             ...
             std::shared_ptr<custom_interfaces::srv::RobPose::Response> res)
    {
      ...
      if(go_to_cart_pose){
        ...
        RCLCPP_INFO(this->get_logger(), "Requested configuration from Cartesian pose: '(%f, %f, %f, %f, %f, %f)'", goal_conf[0][0], goal_conf[0][1], goal_conf[0][2], goal_conf[0][3], goal_conf[0][4], goal_conf[0][5]);
      }
      //reset counter
