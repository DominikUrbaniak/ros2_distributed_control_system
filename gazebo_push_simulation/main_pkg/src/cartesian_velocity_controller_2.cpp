...
class CartesianVelocityPublisher : public rclcpp::Node
{
  ...
  private:
    ...
    void topic_callback(const sensor_msgs::msg::JointState & msg) const
    {
      ...
      tcp_stamp_pose = {(float)control_rate_ms*timeout_counter, (float)stamp_eef.seconds(),transform_ur_eef.transform.translation.x,transform_ur_eef.transform.translation.y,transform_ur_eef.transform.translation.z,tcp_euler.roll,tcp_euler.pitch,tcp_euler.yaw,(float)hlc_counter,(float)timeout_triggered,(float)manipulability_index};
    ...
    void timer_callback()
    {
      ...
      if(go_to_cart_pose){
        ...
        RCLCPP_INFO(this->get_logger(), "go to cart pose");
      }
      else {
      ...
      timeout_counter++;
      RCLCPP_INFO(this->get_logger(), "Timeout triggered: %d,%d,%f", timeout_counter, timeout_steps,vels[0]);
    }
