...
#include "custom_interfaces/srv/get_rob_pose.hpp"
...
int counter = 0;
int hlc_counter = 0;                                     //counting the steps backwards after cube reached the goal
...
class CartesianVelocityPublisher : public rclcpp::Node
{
  public:
    CartesianVelocityPublisher() : Node("cartesian_velocity_publisher")
    {
      ...
      sub_joints_cb_group_ = this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
      exec_service_cb_group_ = this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);

      rclcpp::SubscriptionOptions joints_options;
      joints_options.callback_group = sub_joints_cb_group_;
      subscription_ = this->create_subscription<sensor_msgs::msg::JointState>(
        "/joint_states",
        rate,
        std::bind(&CartesianVelocityPublisher::topic_callback, this, _1),
        joints_options
      );

      rclcpp::SubscriptionOptions vel_options;
      vel_options.callback_group = sub_vel_cb_group_;
      ...
      publisher_ = this->create_publisher<std_msgs::msg::Float64MultiArray>("/forward_velocity_controller_all/commands", 10);
      ...
      service_ = this->create_service<custom_interfaces::srv::RobPose2>("/velocity_controller/set_desired_rob_pose", std::bind(&CartesianVelocityPublisher::set_desired_rob_pose, this, _1, _2),rmw_qos_profile_services_default,service_cb_group_);
      tcp_service_ = this->create_service<custom_interfaces::srv::GetRobPose>("get_tcp_pose", std::bind(&CartesianVelocityPublisher::get_tcp_pose, this,std::placeholders::_1, std::placeholders::_2),rmw_qos_profile_services_default,exec_service_cb_group_);
  ...
  private:
    ...
    rclcpp::Service<custom_interfaces::srv::RobPose2>::SharedPtr service_;
    rclcpp::Service<custom_interfaces::srv::GetRobPose>::SharedPtr tcp_service_;
    std::shared_ptr<tf2_ros::TransformListener> tf_listener_ {nullptr};
    std::unique_ptr<tf2_ros::Buffer> tf_buffer_;
    rclcpp::CallbackGroup::SharedPtr service_cb_group_;
    rclcpp::CallbackGroup::SharedPtr exec_service_cb_group_;
    rclcpp::CallbackGroup::SharedPtr timer_cb_group_;
    rclcpp::CallbackGroup::SharedPtr sub_vel_cb_group_;
    rclcpp::CallbackGroup::SharedPtr sub_joints_cb_group_;
    ...
    void vel_dirs_callback(const custom_interfaces::msg::VelDir & msg) const
    {
      ...
      gripper_pose = msg.goal_dir[6];
      hlc_counter = msg.counter;
      timeout_counter = 0;
      timeout_steps = (float)control_rate/1000*timeout_ms;
      RCLCPP_INFO(this->get_logger(), "Receiving: %d", hlc_counter);
    }
    ...
    void topic_callback(const sensor_msgs::msg::JointState & msg) const
    {
      ...
      tcp_stamp_pose = {(float)timeout_counter, (float)stamp_eef.nanoseconds(),current_joint_pos[0],current_joint_pos[1],current_joint_pos[2],current_joint_pos[3],current_joint_pos[4],current_joint_pos[5],(float)hlc_counter,(float)timeout_triggered,(float)manipulability_index};
    ...
    void get_tcp_pose(std::shared_ptr<custom_interfaces::srv::GetRobPose::Request>  req,
             std::shared_ptr<custom_interfaces::srv::GetRobPose::Response> res)
    {
      res->tcp_pose = {(transform_ur_eef.transform.translation.x*multipliers[0])-ik_offsets[0],(transform_ur_eef.transform.translation.y*multipliers[1])-ik_offsets[1],(transform_ur_eef.transform.translation.z*multipliers[2])-ik_offsets[2],tcp_euler.roll,tcp_euler.pitch,tcp_euler.yaw,gripper_pose};
    }

    void set_desired_rob_pose(std::shared_ptr<custom_interfaces::srv::RobPose2::Request>  req,
             std::shared_ptr<custom_interfaces::srv::RobPose2::Response> res)
    {
      ...
      res->joint_conf = {(float)current_joint_pos[0],(float)current_joint_pos[1],(float)current_joint_pos[2],(float)current_joint_pos[3],(float)current_joint_pos[4],(float)current_joint_pos[5]};
    ...
    void timer_callback()
    {
      ...
      if(go_to_cart_pose){
        ...
        for (int i = 0; i < n_joints; i++)
        {
          //if (i < 6) {
          double vel;
          diffs[i] = requested_joint_configuration[i] - current_joint_configuration[i];
          if(i<=2){
            vel = dev2vel2(diffs[i]); //max speed for joints 1-3 is 180°/sec for UR5e
          }
          else{
            vel = dev2vel2_wrists(diffs[i]); //max speed for wrists is 360°/sec for UR5e
          }
          vels[i] = vel;
        ...
        if(timeout_counter < timeout_steps){
          ...
          for(int i=0;i<n_joints;i++){
            if (i < 6) {
              ...
              diffs[i] = -gripper_pose - current_joint_configuration[i];
        ...
        // get the current joint config to be used in the else loop
        if(timeout_counter == timeout_steps-1){
          got_initial_config = false; // stay at current config until new request arrives
        }
        //RCLCPP_INFO(this->get_logger(), "Timeout counter: %d, timeout_steps: %d", timeout_counter, timeout_steps);
      }
      else{
        timeout_triggered = 1;
        // Calculate velocity for each joint
        for (int i = 0; i < n_joints; i++)
        {
          //if (i < 6) {
            ...
            double vel = dev2vel2(diffs[i]);
            vels[i] = vel;

            message.data.push_back(vel);
        }
        //RCLCPP_INFO(this->get_logger(), "Timeout triggered -> counter: %d, timeout_steps: %d", timeout_counter, timeout_steps);
