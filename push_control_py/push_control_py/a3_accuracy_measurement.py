...
class PoseSubscriberNode(Node):
    ...
    def pose_callback(self, msg):
        ...
        if self.counter < self.n_data_points:
            ...
            #    pass
            #if found_all:
            np_append_tag_poses = np.squeeze(np.reshape(np.array(append_tag_poses),(1,self.measurement_length)))
            #if len(np_append_tag_poses) == self.measurement_length:
                #self.get_logger().info(f'poses:, {np_append_tag_poses}')
            self.all_tag_poses[self.counter,:] = np.array([np_append_tag_poses,n_found_tags])
        else:
