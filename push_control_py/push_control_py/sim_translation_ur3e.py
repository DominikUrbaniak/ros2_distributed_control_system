...
class SimTranslation(Node):
    ...
    def sensing_callback(self, msg):
        ...
        if self.current_episode < self.n_episode:
            ...
            if not self.reset:
                log_message = (
                    f"{self.counter};{self.current_episode};{execution_time:.4f};{accuracy:.4f};{self.latency_sensing};{self.time_stamp_cube_origin};{self.time_stamp_cube_delayed};{self.time_stamp_control};{time_stamp_eef};"
            ...
            with open(output_log_filename, 'w') as log_file:
                log_file.write('Counter;Episode;Execution Time;Accuracy;Induced Latency;Time Stamp Cube Origin;'
