...
#include "rc_reason_msgs/srv/detect_tags.hpp"
#include "rc_reason_msgs/msg/tag.hpp"
...
class RcTagDetectNode : public rclcpp::Node
{
  public:
    RcTagDetectNode() : Node("rc_tag_detect_node")//, state_service_ready_(false), state_received_(false)
    {
      srand ( time(NULL) );
      ...
      timer_ptr_ = this->create_wall_timer(std::chrono::microseconds(frame_rate_microsec), std::bind(&RcTagDetectNode::timer_callback, this),timer_cb_group_);
      client_ptr_ = this->create_client<rc_reason_msgs::srv::DetectTags>("/rc_april_tag_detect_client/detect",rmw_qos_profile_services_default,client_cb_group_);

      //pose_service_ = this->create_service<custom_interfaces::srv::PoseSensing>("/pose_sensing/get_delayed_pose", std::bind(&RcTagDetectNode::get_delayed_pose, this, _1, _2));
      ...
      settings_service_ = this->create_service<custom_interfaces::srv::PoseSensingSettings>("/pose_sensing/set_settings", std::bind(&RcTagDetectNode::set_pose_sesing_settings, this, _1, _2));
  ...
  private:
    ...
    rclcpp::Client<rc_reason_msgs::srv::DetectTags>::SharedPtr client_ptr_;
             ...
             std::shared_ptr<custom_interfaces::srv::PoseSensingSettings::Response> res)
    {
      ...
      RCLCPP_INFO(this->get_logger(), "New settings! Network data file name: %s", network_filename.c_str());
      ...
      if(latency_distribution_size==0){
        RCLCPP_ERROR(this->get_logger(), "Network data not found at %s, continuing without netowork latency", network_filename.c_str());
...
int main(int argc, char * argv[])
{
  ...
  auto gazebo_update_node = std::make_shared<RcTagDetectNode>();
